...
public class ClickIncomeBillActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_click_income_bill);
        ...
        pieChart = findViewById(R.id.income_pieChart);
        recyclerView = findViewById(R.id.income_recyclerView);

        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerView.setAdapter(billAdapter = new BillAdapter(SQLiteUtil.getIncomesForToday()));

        RadioGroup radioGroup = findViewById(R.id.income_radioGroup);
        radioGroup.setOnCheckedChangeListener((group, checkedId) -> {
            if (checkedId == R.id.income_radio_day) {
                updateChartsForDay();
                billAdapter.updateBills(SQLiteUtil.getIncomesForToday());
            } else if (checkedId == R.id.income_radio_week) {
                updateChartsForWeek();
                billAdapter.updateBills(SQLiteUtil.getIncomesForThisWeek());
            } else if (checkedId == R.id.income_radio_month) {
                updateChartsForMonth();
                billAdapter.updateBills(SQLiteUtil.getIncomesForThisMonth());
            }
        });

        // 默认选择日视图
        radioGroup.check(R.id.income_radio_day);
        updateChartsForDay();
        billAdapter.updateBills(SQLiteUtil.getIncomesForToday());
    }

    private void updateChartsForDay() {
        List<Bill> bills = SQLiteUtil.getIncomesForToday();
    ...
    private void updateChartsForWeek() {
        ...
        for (Bill bill : SQLiteUtil.getIncomesForThisWeek()) {
            try {
                Date date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(bill.getTime());
    ...
    private void updateChartsForMonth() {
        ...
        for (Bill bill : SQLiteUtil.getIncomesForThisMonth()) {
            try {
                Date date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(bill.getTime());
